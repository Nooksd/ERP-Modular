...
const Users = ({ toastMessage, modalMessage, modalInfo, openPage }) => {
  ...
  const handleSearch = async (click = false) => {
    try {
      const response = await innovaApi.get(
        `/auth/user/get-all?page=${page}&limit=${limit}&order=${order}&name=${search}&active=${activeMode}`
      );
  ...
  async function deleteUser() {
    try {
      await innovaApi.delete(`/auth/user/delete/${whatDelete}`);
      toastMessage({
        danger: false,
        title: "Sucesso",
  ...
  async function disableUser() {
    try {
      await innovaApi.put(`/auth/user/update/${whatDisable}`, {
        isActive: !activeMode,
      });
